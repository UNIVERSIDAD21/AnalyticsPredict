...
def _fetch_prediccion(cursor: Any, prediction_id: str) -> Optional[Dict[str, Any]]:
    ...
    cursor.execute(
        ...
        SELECT
          ...
          COALESCE(pf.equipo_local, 'LOCAL')::text AS home_team,
          COALESCE(pf.equipo_visitante, 'VISITANTE')::text AS away_team,
          COALESCE(pf.fecha_partido, NOW())::timestamp AS game_date,
          COALESCE(pf.competicion, 'FOOTBALL')::text AS league,
