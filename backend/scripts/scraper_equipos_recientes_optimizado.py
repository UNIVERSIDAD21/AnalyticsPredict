...
# -*- coding: utf-8 -*-
"""
scraper_equipos_recientes_optimizado.py — Versión OPTIMIZADA para velocidad
...
Uso:
    ...
    python scripts/scraper_equipos_recientes_optimizado.py --team "Lakers" --days 30
"""
    ...
    def _load_all_teams(self):
        ...
        with self._lock:
            ...
            try:
                ...
                self._loaded = True  # Marcar como cargado para evitar reintentos infinitos
...
class SummaryCache:
    """Caché de summaries de ESPN por event_id."""
    ...
    def set(self, event_id: str, data: Dict[str, Any]):
        with self._lock:
            self._cache[event_id] = (time.time(), data)
...
def sincronizar_equipo_optimizado(
    ...
    """
    Sincroniza un equipo con ESPN.
    OPTIMIZACIONES:
    - Usa caché de equipos (sin llamada a get_all_teams)
    - Usa caché de summaries
    - Respeta rate limiter
    - Filtra por fecha ANTES de fetch_summary
    """
    stats = SyncStats()
    ...
    for season in seasons:
        ...
        for st in seasontypes:
            ...
            try:
                ...
                session_local = crear_sesion_http()
                events = fetch_schedule_events(session_local, team_id, season, st)
            except Exception as e:
                stats.errores += 1
                continue
            ...
            for ev in events:
                ...
                if summary is None:
                    try:
                        rate_limiter.wait_if_needed()
                        session_local = crear_sesion_http()
                        summary = fetch_summary(session_local, event_id)
                        summary_cache.set(event_id, summary)
                    except Exception as e:
                        stats.errores += 1
                        continue
                ...
                try:
                    ...
                    ins, _pid = upsert_partido_con_fecha(
                        ...
                        visitante_ot=int(parsed["home_ot"]),
...
def sincronizar_equipo_wrapper(
    ...
    """Wrapper para ejecutar en thread separado con su propia conexión."""
    try:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Sincroniza y lista partidos de los últimos N días (VERSIÓN OPTIMIZADA)."
    )
...
def main() -> int:
    ...
    print("=" * 90)
    print("🚀 SCRAPER OPTIMIZADO - AnalyticsPredict")
    print("=" * 90)
    ...
    with psycopg.connect(db_url) as conexion:
        ...
        if not args.no_sync:
            # Inicializar cachés y rate limiter (compartidos)
            ...
            if args.workers > 1 and len(equipos_a_sincronizar) > 1:
                ...
                # Secuencial (para un solo equipo o workers=1)
                for idx, equipo in enumerate(equipos_a_sincronizar, 1):
                    if args.all_teams:
