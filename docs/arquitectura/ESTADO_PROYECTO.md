...
| A3 | EN_CURSO | Auth API en `v2` por defecto + telemetría (`/api/auth/contract-usage`) y Bitácora ya cubre contrato canónico en `listado/resumen/unificada/apuestas-analizadas/estadísticas/métricas/detalle/actualización/eliminación` con `?version=v2|legacy` + telemetría (`GET /api/bitacora/contract-usage`); pruebas de contrato y smoke del alcance actual en verde | Deprecación legacy ya instrumentada en 2 dominios (Auth + Bitácora) con headers estándar (`Deprecation/Sunset/Link`) | Cerrar cobertura v2 de resolución y definir umbral formal de sunset global |
