...
| B3 | EN_CURSO | Cross-liga v1 activo en análisis (`servicios/b3_estabilizacion_futbol.py` + `api/rutas_analisis_futbol.py`), confianza ajustada por muestra, y nuevo gate semanal por liga en `GET /api/futbol/metricas/b3-estabilidad` (clasifica estable/warning/critico por delta de Brier semanal vs semana previa) con pruebas unitarias de clasificación | B3 se gobierna por calidad semanal objetiva (Brier) y regla de no degradación crítica por liga antes de cierre | Ejecutar 2 ciclos semanales consecutivos sin ligas en estado crítico y publicar evidencia en reportes operativos |
