...
| B4 | EN_CURSO | Backend B4 inicial implementado: preferencias de notificación (`GET/PUT /api/notificaciones/preferencias`), envío de prueba por email (`POST /api/notificaciones/enviar-prueba`) e historial (`GET /api/notificaciones/historial`) con persistencia SQLite (`servicios/notificaciones_store.py`) y pruebas API | Se prioriza retención por notificación controlada y trazabilidad de envíos antes de automatizar jobs | Integrar jobs programados por tipo de alerta, conectar preferencias en frontend y añadir reintentos/cola para fallos SMTP |
