...
| B2 | EN_CURSO | Onboarding y dashboard operativos: wizard frontend (`/onboarding`) + dashboard (`/dashboard`) y ahora persistencia backend en `POST/GET /api/onboarding/perfil|estado` (SQLite/PostgreSQL por driver), además de telemetría de conversión (`POST /api/onboarding/evento`: onboarding_started/completed, dashboard_viewed) | Onboarding obligatorio antes de uso de rutas principales; medición explícita de activación para decisiones de retención | Cerrar UX/copy final de retención, agregar visualización de embudo de conversión en dashboard interno y definir metas cuantitativas (completion rate, time-to-value) |
