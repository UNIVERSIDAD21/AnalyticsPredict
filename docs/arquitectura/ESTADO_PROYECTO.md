...
| A3 | EN_CURSO | Auth API en `v2` por defecto + telemetría (`/api/auth/contract-usage`) y ahora Bitácora inició contrato canónico con `?version=v2|legacy` en listado/resumen + telemetría (`GET /api/bitacora/contract-usage`); pruebas de contrato y smoke 6/6 passing en alcance actual | Deprecación legacy ya instrumentada en 2 dominios (Auth + Bitácora) con headers estándar (`Deprecation/Sunset/Link`) | Extender cobertura v2 al resto de endpoints de Bitácora (detalle/estadísticas/métricas) y definir umbral formal de sunset global |
