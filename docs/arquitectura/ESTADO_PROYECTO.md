...
| A4 | EN_CURSO | Registro exige aceptación legal versionada y persistida; se añadió endpoint autenticado `POST /api/auth/accept-legal` para regularizar usuarios legacy y actualizar `legal_accepted_version/legal_accepted_at`; frontend ya publica `/legal/*` y consentimiento en alta | Legal transversal obligatorio con versionado explícito | Endurecer guard de flujos críticos para bloquear operación cuando `legal_accepted_version` no coincida con la vigente |
