...
| B1 | CERRADO (equivalente trazable C1) | Checkout + webhook firmado + matriz de estados + idempotencia de eventos + feature-gate consistente por suscripción. Endpoints: `POST /api/pagos/checkout-session`, `POST /api/pagos/webhook/mercadopago`, `GET /api/pagos/suscripcion/mia`, `GET /api/pagos/feature-gate`, `GET /api/pagos/matriz-estados`. Evidencia E2E reproducible en `docs/reportes/C1_E2E_PRIMER_PAGO_VALIDACION_EQUIVALENTE_2026-03-24.md` con `backend/.venv/bin/pytest -q backend/tests/api/test_pagos_endpoints.py` (6/6). | MercadoPago gateway inicial con webhook idempotente y trazabilidad operativa | Riesgo residual pasa a C2: persistencia SQLite launch-critical requiere inventario y decisión formal de migración/aceptación para go-live definitivo |
