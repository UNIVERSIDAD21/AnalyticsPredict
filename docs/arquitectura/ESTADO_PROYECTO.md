...
| A3 | EN_CURSO | Auth API ya quedó en `v2` por defecto (legacy explícito), frontend auth migrado a v2 y cobertura de pruebas para ambos contratos (10/10 passing en suite auth+smoke) | Estrategia de deprecación controlada: legacy solo bajo `?version=legacy` + headers y telemetría | Extender patrón canónico al siguiente dominio crítico (Bitácora/Explicabilidad) y preparar retiro legacy por adopción |
