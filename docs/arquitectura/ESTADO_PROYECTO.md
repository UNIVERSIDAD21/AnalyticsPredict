...
| A3 | EN_CURSO | Auth API en `v2` por defecto + telemetría (`/api/auth/contract-usage`) y Bitácora ya cubre contrato canónico en `listado/resumen/estadísticas/métricas/detalle/actualización/eliminación` con `?version=v2|legacy` + telemetría (`GET /api/bitacora/contract-usage`); pruebas de contrato y smoke 6/6 passing en alcance actual | Deprecación legacy ya instrumentada en 2 dominios (Auth + Bitácora) con headers estándar (`Deprecation/Sunset/Link`) | Extender cobertura v2 a endpoints restantes de Bitácora (unificada/apuestas-analizadas/resolución) y definir umbral formal de sunset global |
