...
| A3 | EN_CURSO | Auth API en `v2` por defecto + endpoint de telemetría de adopción (`GET /api/auth/contract-usage`) con resumen legacy/v2 por días; pruebas auth+smoke 11/11 passing | Deprecación legacy medible y auditable para decidir retiro gradual | Extender el patrón canónico al siguiente dominio crítico (Bitácora/Explicabilidad) y fijar umbral formal de sunset |
