...
| B3 | EN_CURSO | Inicio técnico B3: combinación cross-liga por competición en motor de análisis (`servicios/b3_estabilizacion_futbol.py` + integración en `api/rutas_analisis_futbol.py`), ajuste de probabilidad por muestra total/relevante y confianza B3; pruebas unitarias iniciales agregadas | Se formaliza estrategia de estabilización por muestra relevante (evita sobreconfianza en mercados con bajo contexto) | Completar policy gate semanal por drift/calidad y validar 2 ciclos sin degradación crítica sostenida |
