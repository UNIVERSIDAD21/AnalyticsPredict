...
| A5 | EN_CURSO | Instrumentación HTTP mínima en backend: middleware con captura de latencia/status + endpoint `/api/interno/observabilidad-http` (p95/error rate/uptime con umbrales y alertas básicas) | Métricas mínimas operativas obligatorias | Completar scraping health en dashboard operativo y validar alertas en prueba controlada end-to-end |
